...
run:
	docker run -p 8140:8140 --hostname puppetmaster --name $(subst /,-,$(NAME)) --restart=always -t --cidfile cidfile -d $(NAME):$(VERSION) /sbin/runit
...
version:
	...
	docker run -it --rm $(NAME):$(VERSION) sh -c " echo -n puppet: ; puppet --version " | tee -a COMPONENTS
	dos2unix COMPONENTS
